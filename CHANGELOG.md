...
### Added
- Step 2 新增“📊 榜单人工审核”区：研究完成后立即展示从 Wikipedia 抓到的
  全部榜单（与 Step 2 评分徽章同级），每条可编辑榜单名/排名、可删除、
  可添加；人工修改后的榜单直接用于卡片，不用跑完整流程才发现错误。
- 换歌防护：切换音频文件后，若研究数据仍属于上一首歌（艺人不同），
  自动清空研究数据与榜单审核区并提示重新搜索，
  避免出现“Kelsey Lu 卡片却带 2 Chainz 榜单”的问题。

## [3.29.11] - 2026-08-08

### Fixed
- 热度不再使用评分时的旧快照：自动爬取值单独存
  （`window._autoHeatData`），生成卡片时用“自动 + 当前手动输入”
  重新计算热度（QQ/网易云评论收藏/YouTube 播放/RYM 人数手动修改立即生效）。
- Step 2 手动平台输入（QQ/网易云/YouTube/RYM 人数）改为实时重算热度并
  刷新国内★/国外● 徽章；输入框标注“留空=自动，可手动修正”。
